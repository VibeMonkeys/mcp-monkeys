...
class StructuredResponseService(
    ...
    /**
     * 구조화된 채팅 응답 생성 (Spring AI 2.0 ChatMemoryAdvisor 활용)
     */
    fun generateStructuredResponse(request: ChatRequest): StructuredChatResponse {
        ...
        try {
            ...
            val rawResponse = chatResponse.content()?.trim()
                ?: throw ChatServiceException("모델이 비어 있는 응답을 반환했습니다.", "EMPTY_RESPONSE")

            val validatedResponse: Any = try {
                converter.convert(rawResponse)
            } catch (ex: Exception) {
                logger.error("구조화된 응답 파싱 실패: sessionId=$sessionId", ex)
                throw ChatServiceException("응답을 구조화된 형식으로 변환할 수 없습니다: ${ex.message}", "STRUCTURED_PARSING_FAILED", ex)
            } ?: throw ChatServiceException("구조화된 응답이 비어 있습니다.", "STRUCTURED_EMPTY_RESULT")

            logger.debug("구조화된 응답 생성 완료: type=${responseType.simpleName}, sessionId=$sessionId")

            val estimatedTokens = chatResponseHelper.estimateTokenCount(structuredPrompt + rawResponse)
